...
namespace SocialNetwork.Service
{
    public interface IPostService
    {
        ...
        Task<PayLoad<object>> FindAll(string? name, int? category, PostSortBy sortBy = PostSortBy.Newest, int page = 1, int pageSize = 20);
        Task<PayLoad<object>> FindAllPostByUser(string? name, int? category, PostSortBy sortBy = PostSortBy.Newest, int page = 1, int pageSize = 20);
